        ...
        for i in range(len(element) - 1):
            for j in range(i + 1, len(element)):
                ...
                if tmp in list(set(item) for item in currentDict.keys()):
                    print('tmp',tmp)
                    print('list(set(item) for item in currentDict.keys())', list(set(item) for item in currentDict.keys()))
                    continue
        ...
        for item in self.dataset:
            for i in range(len(item)):
                if item[i] in list(count[0].keys()):
                    ...
                    count[0][item[i]] = 1
                    # print(item[i],count[0].keys())
            self.num += 1  # 统计一共有多少个元素
        # count [{'e': 4, 'h': 2, 'c': 4, 'f': 2, 'i': 3, 'k': 2, 'b': 4, 'd': 1, 'j': 3, 'g': 2}]
        # print(self.num)
        # print(count)
            ...
            for itemset in list(self.count[i].keys()):
                ...
                for i in range(len(subset) - 1):
                    for j in range(i + 1, len(subset)):
                        ...
                        if len(subset[i]) + len(subset[j]) == len(itemset) and len(
                                set(subset[i]) & set(subset[j])) == 0:
